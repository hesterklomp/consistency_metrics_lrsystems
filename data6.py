...
# Open data
with open('C:\\Users\\heste\\OneDrive\\Documents\\TU Delft\\THESIS\\Data + Code\\data_thesis\\data6\\lrs_huls_paren.csv',
          newline='') as csvfile:
    ...
    for row in csv_reader:
            ...
            if label == '1.0':
                if lr_value.lower() == 'inf':
                    data_SS.append(float('inf'))
                else:
                    data_SS.append(float(lr_value))
            elif label == '0.0':
                if lr_value.lower() == 'inf':
                    data_DS.append(float('inf'))
                else:
                    data_DS.append(float(lr_value))


# Convert the values in log_data_SS to log10
log_data_SS = [math.log10(value) if value != float('inf') else float('inf') for value in data_SS]

# Convert the values in log_data_DS to log10
log_data_DS = [math.log10(value) if value != float('inf') else float('inf') for value in data_DS]
...
# Calculate overlap percentage and plot
try:
    ...
    print(f"Error in calculating devPAV overlap: {e}")

# Collect cllr results
results_cllr = {
    'Perfect': cllr_p,
    'Right c=1': cllr_r1,
    'Left c=1': cllr_l1,
...
# Calculate overlap percentage and plot results
try:
    ...
    print(f"Error in calculating CLLR overlap: {e}")
...
try:
    ...
    print(f"Error in calculating Fid overlap: {e}")