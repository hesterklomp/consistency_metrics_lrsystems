...
# Initialize scaling factors and amount of data
c_1 = 1
c_2 = 2
d_1 = 1.5
d_2 = 2.5
...
# Initialize amount of times to compute metric
N = 1000
...
# Unpack data
log_data_SS = []
with open('C:\\Users\\heste\\OneDrive\\Documents\\TU Delft\\THESIS\\Data + Code\\data_thesis\\data3\\LLR_KM.csv', newline='') as csvfile:
...
log_data_DS = []
with open('C:\\Users\\heste\\OneDrive\\Documents\\TU Delft\\THESIS\\Data + Code\\data_thesis\\data3\\LLR_KNM.csv', newline='') as csvfile:
    csv_reader = csv.reader(csvfile)

    for row in csv_reader:
        log_data_DS.append(float(row[0]))

print('SS size: ', len(log_data_SS))
print('DS size: ', len(log_data_DS))
...
# Determine array of LR's and corresponding KDE based on DS data to find distribution
x_values = np.linspace(min(log_data_DS), max(log_data_DS), 10000)
kde_original = gaussian_kde(log_data_DS, bw_method='scott')

# Plot the KDE of the DS data
sns.kdeplot(log_data_DS, fill=False, bw_method='scott', color='red')
plt.xlabel('LLR', fontsize=12)
plt.ylabel('Density', fontsize=12)
plt.title('Kernel Density Estimate DS data', fontsize=16)
plt.show()

# Generate a consistent LR-system using the DS data
...
# Plot new distributions of consistent data
plt.figure(figsize=(10, 6))
plt.plot(stretched_LLRs, LDS_frequencies, label='LDS Frequencies', color='red')
plt.plot(stretched_LLRs, LSS_frequencies, label='LSS Frequencies', color='blue')
plt.title('Generated SS and DS distribution', fontsize=16)
plt.xlabel('Stretched LLRs', fontsize=12)
plt.ylabel('Frequency', fontsize=12)
plt.legend()
plt.show()

for i in range(N):
    # Take samples from consistent distributions of given sizes
    ...
    min_bin = min(min(samples_LSS), min(samples_LDS))
    max_bin = max(max(samples_LSS), max(samples_LDS))
    bins = np.linspace(min_bin, max_bin, 15)


    plt.hist(samples_LSS, bins=bins, color='blue', density=True, alpha=0.5, label='SS')
    plt.hist(samples_LDS, bins=bins, color='red', density=True, alpha=0.5, label='DS')
    plt.xlabel('LLR', fontsize=12)
    plt.ylabel('Probability Density', fontsize=12)
    plt.legend()
    ...
    # Create data skewed to the right
    ...
    # Calculate the metrics for skewed data
    ...
    # Create data skewed to the left
    ...
    # Calculate metrics for too extreme data
    ...
    # Calculate the metrics for weak data
...
# Collect devPAV results in dictionary
results_dev = {
    'Perfect': dev_p,
    'Right c=1': dev_r1,
    'Left c=1': dev_l1,
...
# Collect cllr values in dictionary
results_cllr = {
    'Perfect': cllr_p,
    'Right c=1': cllr_r1,
    'Left c=1': cllr_l1,
...
# Collect fid results in dictionary
results_fid = {
    'Perfect': fid_p,
    'Right c=1': fid_r1,
    'Left c=1': fid_l1,
